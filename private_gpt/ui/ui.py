...
class PrivateGptUi:
    ...
    def _chat(
        ...
        def yield_deltas(completion_gen: CompletionGen) -> str:
            full_response: str = ""
        ...
        def yield_tokens(token_gen: TokenGen) -> str:
            full_response: str = ""
        ...
        match mode:
            case Modes.RAG_MODE:
                ...
                response_text = yield_deltas(query_stream)
            case Modes.BASIC_CHAT_MODE:
                llm_stream = self._chat_service.stream_chat(
                    messages=all_messages,
                    use_context=False,
                )
                response_text = yield_deltas(llm_stream)
            ...
            case Modes.SUMMARIZE_MODE:
                ...
                response_text = yield_tokens(summary_stream)
    ...
    def _set_current_mode(self, mode: Modes) -> list[gr.update]:
        self.mode = mode
    ...
    def _delete_all_files(self) -> list[gr.update]:
        ...
        return [
            gr.update(value=self._list_ingested_files()),
            gr.update(interactive=False),
            gr.update(interactive=False),
            gr.update(value="All files"),
        ]

    def _delete_selected_file(self) -> list[gr.update]:
    ...
    def _deselect_selected_file(self) -> list[gr.update]:
        self._selected_filename = None
        return [
            gr.update(interactive=False),
            gr.update(interactive=False),
    ...
    def _selected_a_file(self, select_data: gr.SelectData) -> list[gr.update]:
            ...
            with gr.Row(equal_height=False):
                with gr.Column(scale=3):
                    ...
                    ingested_dataset = gr.List(
                        value=self._list_ingested_files(),
                        headers=["File name"],
                    ...
                    ingested_dataset.change(
                        fn=lambda: gr.update(value=self._list_ingested_files()),
                        outputs=ingested_dataset,
                    )
                ...
                with gr.Column(scale=7, elem_id="col"):
                    ...
                    submit.click(
                        self._chat,
                        inputs=[msg, chatbot, mode, upload_button, system_prompt_input],
                        outputs=[chatbot, msg],
                        clear_input=True,
                    )
    ...
    def mount_in_app(self, app: FastAPI, path: str) -> None:
        ...
        blocks.mount_to_app(app, path=path, favicon_path=AVATAR_BOT)
